...
/**
 * A replacement string is a small language, and content must never be written in it.
 *
 * `String.replace(pattern, replacement)` reads $& as the match, $` and $' as the
 * text either side, and $1 as a capture group. Every page here was assembled by
 * substituting content into a shell that way, which meant any article
 * containing those two characters was silently rewritten on its way out.
 *
 * One did. "A proof either way carries a prize of $1,000,000" reached crawlers
 * as ",000,000" — the $1 read as the first capture group and expanded to the
 * container tag. Correct on screen, wrong in the HTML, and wrong in exactly the
 * place nobody looks. It was found by a reader checking the source.
 *
 * The rule is not "escape the dollars". It is that content is data: every
 * substitution takes a function, which turns the language off entirely.
 */
describe('content is never a replacement pattern', () => {
  const RENDER = readFileSync(join(ROOT, 'scripts/render.js'), 'utf8');
  const PRE = readFileSync(join(ROOT, 'scripts/prerender.js'), 'utf8');

  test('the figure survives, in the prerendered body and in the payload', () => {
    const a = payload.articles.find(x => x.slug === 'smooth-until-proven-otherwise');
    assert.ok(a, 'the article that found this is still published');
    assert.ok(a.html.includes('$1,000,000'), 'the payload lost the figure');
    const body = prerender(`/article/${a.slug}`).html;
    assert.ok(body.includes('$1,000,000'), 'the prerendered body lost the figure');
    assert.ok(!/[^1],000,000/.test(body), 'the prerendered body carries a truncated figure');
  });

  test('any article may say $1, $&, $` or $\u0027 without being rewritten', () => {
    /* The next one to trip this will not be about a Clay prize. */
    const probes = ['$1,000,000', '$& per litre', 'cost $`x`', "up to $'"];
    for (const probe of probes) {
      const shell = '<div id="articleBody"></div>';
      const out = shell.replace(/(<div[^>]*id="articleBody"[^>]*>)/, m => m + probe);
      assert.ok(out.includes(probe), `${probe} did not survive`);
    }
  });

  test('nothing bearing content is passed as a replacement string', () => {
    /* Structural: the two files that assemble pages use function replacers. */
    assert.match(PRE, /\.replace\(opened, m => m \+ content\)/);
    assert.match(RENDER, /const put = \(hay, needle, value, all = false\)/);
    assert.ok(!/\.replace\('__PAYLOAD__', json\)/.test(RENDER),
      'the payload is still substituted as a replacement string');
  });
});

/**
 * Reading time is derived, and an editor may overrule it.
 *
 * The site computes the figure from the prose it publishes, excluding the
 * source list because a bibliography is looked up rather than read at 220 words
 * a minute. That default is right and it is not always the answer: a piece
 * dense with equations reads slower than its word count, and the editor knows
 * that where the counter does not.
 *
 * Setting `minutes` in the frontmatter is therefore a declaration rather than a
 * preference. It is recorded as one, so the two cases stay distinguishable —
 * a number somebody chose, and a number that follows the text.
 */
describe('reading time is one number with a stated origin', () => {
  test('every piece says where its figure came from', () => {
    for (const a of payload.articles) {
      assert.ok(['derived', 'declared'].includes(a.minutes_source), `${a.slug}: ${a.minutes_source}`);
      assert.ok(Number.isInteger(a.minutes) && a.minutes > 0, `${a.slug}: ${a.minutes}`);
    }
  });

  test('a declared figure is honoured, and a derived one follows the words', () => {
    const declared = payload.articles.filter(a => a.minutes_source === 'declared');
    for (const a of declared) assert.notEqual(a.minutes, Math.ceil(a.words / a.reading_wpm),
      `${a.slug} declares a figure identical to the derived one — the declaration is doing nothing`);
    for (const a of payload.articles.filter(a => a.minutes_source === 'derived')) {
      assert.equal(a.minutes, Math.max(1, Math.ceil(a.words / a.reading_wpm)), a.slug);
    }
  });

  test('a declaration that is not a number is not a declaration', () => {
    /* Frontmatter arrives as text. "eight" must fall back, not ship NaN. */
    assert.match(readFileSync(join(ROOT, 'src/markdown.js'), 'utf8'),
      /const declared = Number\(meta\?\.minutes\)/);
  });
});

/**
 * The mark is a circle, and the file has to be one too.
 *
 * The PNGs were fully opaque: a dark circle sitting on a white square, inset by
 * about two per cent. Against the site's warm white (#F8F8F5) the square showed
 * as a pale panel behind the logo. Rounding it in CSS would have hidden the
 * corners and left a white ring at the edge; the asset was wrong, so the asset
 * was fixed.
 *
 * These read the files. An exported mark that loses its transparency again
 * fails here rather than shipping.
 */
describe('the brand mark is transparent outside the circle', () => {
  const marks = ['twj-mark-64.png', 'twj-mark-160.png', 'twj-mark-512.png'];

  /** Minimal PNG reader: signature, IHDR, and the alpha of the four corners
      is more than this needs — colour type 6 (RGBA) is the whole check. */
  const ihdr = file => {
    const b = readFileSync(join(ROOT, 'content/brand', file));
    assert.equal(b.toString('ascii', 12, 16), 'IHDR', `${file} is not a PNG`);
    return { w: b.readUInt32BE(16), h: b.readUInt32BE(20), colourType: b[25] };
  };

  test('every mark carries an alpha channel', () => {
    for (const f of marks) {
      const { colourType } = ihdr(f);
      assert.equal(colourType, 6, `${f} has colour type ${colourType}, not RGBA — it cannot be transparent`);
    }
  });

  test('every mark is square, at the size its name claims', () => {
    for (const f of marks) {
      const { w, h } = ihdr(f);
      assert.equal(w, h, `${f} is ${w}x${h}`);
      assert.equal(String(w), /-(\d+)\.png$/.exec(f)[1], `${f} is ${w}px`);
    }
  });
});
