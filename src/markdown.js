...
/**
 * Reading time, derived — unless an editor overrides it.
 *
 * The figure is computed from the prose the page publishes, with the source list
 * excluded because a bibliography is looked up rather than read. That is the
 * right default and it is not always the right answer: a piece dense with
 * equations or a table reads slower than its word count says, and the editor
 * who wrote it knows that and the counter does not.
 *
 * So `minutes` may be set in the frontmatter. Doing so is a declaration, not a
 * preference: it means somebody decided, and the number stops moving when the
 * text is edited. Left unset, it is derived and always current.
 */
export const minutesFor = (meta, prose) => {
  /* Frontmatter values arrive as strings; a declaration has to be a number
     before it can be believed, and a value that is not a number is not a
     declaration — it falls back rather than shipping NaN. */
  const declared = Number(meta?.minutes);
  return Number.isFinite(declared) && declared > 0
    ? { minutes: declared, minutes_source: 'declared' }
    : { minutes: readingMinutes(prose), minutes_source: 'derived' };
};
